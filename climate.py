...
def create_climate_entity(tado, name: str, zone_id: int, device_info: dict):
    ...
    if zone_type == TYPE_AIR_CONDITIONING:
        ...
        for mode in ORDERED_KNOWN_TADO_MODES:
            ...
            if "fanSpeeds" in capabilities[mode]:
    ...
    # _LOGGER.debug('=======')
    # _LOGGER.debug(name)
    # _LOGGER.debug(str(hvac_capability_map))
    # _LOGGER.debug('=======')
...
class TadoClimate(TadoZoneEntity, ClimateEntity):
    ...
    def _control_hvac(
        ...
        if self.supported_features & ClimateEntityFeature.FAN_MODE:
            if TADO_TO_HA_FAN_MODE_MAP[self._current_tado_fan_speed] in self.fan_modes:
